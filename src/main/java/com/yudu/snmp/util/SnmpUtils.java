...
public class SnmpUtils {
    ...
    //获取cpu使用率
    public static String cpuRateUse(String ip){
        ...
        try {
            ...
            if(list.size()==1 && list.get(0).getColumns()==null){
                ...
                return "CPU利用率为：" + percentage / list.size() + "%"+"\r\n";
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    //获取内存使用率
    public static String memoryRateUse(String ip){
        String result = "";
            ...
            if(list.size()==1 && list.get(0).getColumns()==null){
                ...
                for(TableEvent event : list){
                    ...
                    if (PHYSICAL_MEMORY_OID.equals(oid)){
                        result+= "PHYSICAL_MEMORY----->物理内存大小："+(long)totalSize * unit/(1024*1024*1024)+"G   内存使用率为："+(long)usedSize*100/totalSize+"%"+ "\r\n";
                    }else if (VIRTUAL_MEMORY_OID.equals(oid)) {
                        result+= "VIRTUAL_MEMORY----->虚拟内存大小："+(long)totalSize * unit/(1024*1024*1024)+"G   内存使用率为："+(long)usedSize*100/totalSize+"%"+ "\r\n";
                    }
                }
            }
        } catch(Exception e){
    ...
    //获取磁盘使用率
    public static String diskRateUse(String ip){
        String result = "";
        TransportMapping transport = null;
        Snmp snmp = null ;
        CommunityTarget target;
        ...
        try {
            ...
            if(list.size()==1 && list.get(0).getColumns()==null){
                ...
                for(TableEvent event : list){
                    ...
                    int totalSize = Integer.parseInt(values[3].getVariable().toString());//size 总存储单元数
                    int usedSize = Integer.parseInt(values[4].getVariable().toString());//used  使用存储单元数
                    result+= getChinese(values[1].getVariable().toString()).split(":")[0]+"   磁盘大小："+(long)totalSize*unit/(1024*1024*1024)+"G   磁盘使用率为："+(long)usedSize*100/totalSize+"%"+ "\r\n";
                }
            }
        } catch(Exception e){
            e.printStackTrace();
    ...
    //获取服务器进程信息
    public static String processInfo(String ip){
        String result = "";
        TransportMapping transport = null ;
        Snmp snmp = null ;
        CommunityTarget target;
        ...
        try {
            ...
            if(list.size()==1 && list.get(0).getColumns()==null){
                ...
                for(TableEvent event : list){
                    ...
                    result+= "name--->"+name+"  cpu--->"+cpu+"  memory--->"+memory+"  path--->"+path+"\r\n";
                }
            }
        } catch(Exception e){
            e.printStackTrace();
    ...
    //获取端口信息
    public static String portInfo(String ip){
        String result ="";
        TransportMapping transport = null ;
        Snmp snmp = null ;
        CommunityTarget target;
        ...
        try {
            ...
            if(list.size()==1 && list.get(0).getColumns()==null){
                ...
                for(TableEvent event : list){
                    ...
                    result+= "status--->"+status+"   TCP_port--->"+values[1].getVariable().toString()+"\r\n";
                }
            }
            //获取udp 端口
